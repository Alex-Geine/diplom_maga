...
\usepackage{titlesec} % Для центрирования
\titleformat{\section}
  {\normalfont\normalsize\bfseries\centering} % стиль: шрифт, размер, жирный, центр
  {\thesection}                          % ярлык (номер)
  {1em}                                  % расстояние от номера до текста
  {}                                     % код перед заголовком

% Настройка отступов (обязательно при ошибке Format/spacing not found)
% {команда}{левый отступ}{отступ сверху}{отступ снизу}
% Сделаем минимальные отступы, например, 2 строки сверху и 1 снизу
\titlespacing*{\section}
  {0pt}                % Отступ слева
  {6pt plus 4pt minus 2pt} % Отступ сверху (был 3.5ex, стал значительно меньше)
  {6pt plus 4pt minus 2pt}  % Отступ снизу (был 2.3ex)
...
% Если класс disser по умолчанию запрещает переносы — переопределим штрафы
\hyphenpenalty=10        % разрешаем переносы (чем меньше, тем охотнее)
\tolerance=200           % допустимая растяжка межсловных пробелов
\emergencystretch=3em    % запас на случай длинных слов